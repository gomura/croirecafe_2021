...
class PluginManager extends AbstractPluginManager
{
    ...
    public function enable(array $config, ContainerInterface $container)
    {
        ...
        // 支払方法名称変更（「クロネコ代金後払い(請求書SMS送付)」->「クロネコ代金後払いサービス(スマホタイプ)」）
    ...
    public function disable(array $config, ContainerInterface $container)
    {
        ...
        $methods = [
            ...
            'クロネコ代金後払いサービス(スマホタイプ)' => DeferredSms::class,
        ];
    ...
    /**
     * 支払方法名称変更
     * 「クロネコ代金後払い(請求書SMS送付)」->「クロネコ代金後払いサービス(スマホタイプ)」
     */
    protected function updatePaymentMethod()
    {
        // plg_yamato_payment_method
        $yamatoPaymentMethodRepository = $this->entityManager->getRepository(YamatoPaymentMethod::class);
        $YamatoPaymentMethod = $yamatoPaymentMethodRepository->findOneBy(['payment_method' => 'クロネコ代金後払い(請求書SMS送付)']);
        if ($YamatoPaymentMethod) {
            $YamatoPaymentMethod->setPaymentMethod('クロネコ代金後払いサービス(スマホタイプ)');
            $this->entityManager->persist($YamatoPaymentMethod);
            $this->entityManager->flush();
        }
        ...
        $Payment = $paymentRepository->findOneBy(['method_class' => DeferredSms::class]);
        if ($Payment) {
            $Payment->setMethod('クロネコ代金後払いサービス(スマホタイプ)');
