...
class BaseClientService
{
        ...
        foreach ($sendKey as $key) {
            switch ($key) {
                ...
                case 'buyer_name_kanji':
                    $sendData[$key] = $Order->getName01().'　'.$Order->getName02();
                    break;
                case 'buyer_name_kana':
                    $sendData[$key] = $Order->getKana01().'　'.$Order->getKana02();
                    break;
                case 'buyer_tel':
    ...
    {
        // コンビニ名を取得
        $csv_name = $this->getCvsName($cvsId);
        $yamatoOrderInfo = $yamatoOrder->getMemo02();

        // 受注情報にコンビニ名を追加
        $yamatoOrderInfo['cvs_name'] = array(
            'name' => 'コンビニ名',
            'value' => $csv_name
        );
...
EOM;
     ...
     * @return コンビニ名
     */
    private function getCvsName($cvsId)
    {
        switch ($cvsId) {
            case $this->eccubeConfig['CONVENI_ID_SEVENELEVEN']:
                return $this->eccubeConfig['CONVENI_NAME_SEVENELEVEN'];
                break;
            case $this->eccubeConfig['CONVENI_ID_LAWSON']:
                return $this->eccubeConfig['CONVENI_NAME_LAWSON'];
                break;
            case $this->eccubeConfig['CONVENI_ID_FAMILYMART']:
                return $this->eccubeConfig['CONVENI_NAME_FAMILYMART'];
                break;
            case $this->eccubeConfig['CONVENI_ID_SEICOMART']:
                return $this->eccubeConfig['CONVENI_NAME_SEICOMART'];
                break;
            case $this->eccubeConfig['CONVENI_ID_MINISTOP']:
                return $this->eccubeConfig['CONVENI_NAME_MINISTOP'];
                break;
            case $this->eccubeConfig['CONVENI_ID_CIRCLEK']:
                return $this->eccubeConfig['CONVENI_NAME_CIRCLEK'];
                break;
        }
    }
}
