...
class CreditClientService extends BaseClientService
{
    ...
    public function doRegistCard($Customer, $dummyOrder, $arrParam = [])
    {
        ...
        // 支払方法設定：本人認証サービス(3Dセキュア) 0:利用しない 1:利用する
        $arrParam['auth_div'] = ($this->paymentInfo['TdFlag'] == '0') ? '2' : '1';

        if ($arrParam['auth_div'] == '1') {
            // 認証区分が「1：３Ｄセキュアあり」
            $arrSendKey[] = 'trader_ec_url'; //加盟点ECサイトURL
            $arrParam['trader_ec_url'] = $this->router->generate('mypage_kuronekocredit', ['mode' => '3dTran', 'orderId' => $dummyOrder->getId()], UrlGeneratorInterface::ABSOLUTE_URL);
        }
